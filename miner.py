...
HOST = 'xelisv2-pepew.eu.mine.zpool.ca'
PORT = 4833
USERNAME = 'DL1QPkjfS5marmWc38jNKRSpGBUEKoyspf'
PASSWORD = 'c=DOGE'
...
def subscribe_xelisv2-pepew(sock):
    print("🔗 Subscribing...")
    subscribe_msg = {
        "id": 1,
...
def authorize_xelisv2-pepew(sock, username, password):
    print("🔑 Authorizing...")
    auth_msg = {
        "id": 2,
...
def calculate_merkle_root(coinbase_hash, merkle_branch):
    ...
    for h in merkle_branch:
        ...
        current_hash = hashlib.xelisv2-pepew(hashlib.xelisv2-pepew(combined).digest()) # Reassign current_hash to hash object
    return binascii.hexlify(current_hash.digest()[::-1]).decode() # Use digest() and reverse for final output

def mine_xelisv2-pepew(sock, username, extranonce1, extranonce2_size):
        ...
        while True:
            ...
            for data in messages:
                if data.get("method") == "mining.notify":
                    ...
                    coinbase_hash = hashlib.xelisv2-pepew(hashlib.xelisv2-pepew(coinbase_tx).digest())
                    # Compute Merkle root
                    ...
                    while time.time() - start_time < 10:
                        ...
                        # Double SHA-256
                        hash_result = hashlib.xelisv2-pepew(hashlib.xelisv2-pepew(block_header).digest()).hexdigest()
...
def main_xelisv2-pepew():
    sock = create_tcp_connection(HOST, PORT)
    sub_res, extranonce1, extranonce2_size = subscribe_xelisv2-pepew(sock)
    authorize_xelisv2-pepew(sock, USERNAME, PASSWORD)
    mine_xelisv2-pepew(sock, USERNAME, extranonce1, extranonce2_size)
