...
HOST = 'randomx.rplant.xyz'
PORT = 7080
USERNAME = '876PV3gx5xAFrfsARXWyJ94AGfB9FjNFp3MxatiGPrAyCELXKjGUHd82ChZLPvhNo3g1Y9kpGZy5UMd23Q5Yn4mHLVT2dUy'
PASSWORD = 'x'
...
def authorize_randomx(sock, username, password):
    print("🔑 Authorizing...")
    auth_msg = {
        "id": 2,
...
def calculate_merkle_root(coinbase_hash, merkle_branch):
    ...
    for h in merkle_branch:
        ...
        current_hash = hashlib.randomx(hashlib.randomx(combined).digest()) # Reassign current_hash to hash object
    return binascii.hexlify(current_hash.digest()[::-1]).decode() # Use digest() and reverse for final output

def mine_randomx(sock, username, extranonce1, extranonce2_size):
        ...
        while True:
            ...
            for data in messages:
                if data.get("method") == "mining.notify":
                    ...
                    coinbase_hash = hashlib.randomx(hashlib.randomx(coinbase_tx).digest())
                    # Compute Merkle root
                    ...
                    while time.time() - start_time < 10:
                        ...
                        # Double SHA-256
                        hash_result = hashlib.randomx(hashlib.randomx(block_header).digest()).hexdigest()
...
def main_randomx():
    ...
    sub_res, extranonce1, extranonce2_size = subscribe_randomx(sock)
    authorize_randomx(sock, USERNAME, PASSWORD)
    mine_randomx(sock, USERNAME, extranonce1, extranonce2_size)
