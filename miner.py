...
def authorize_kawpow(sock, username, password):
    print("🔑 Authorizing...")
    auth_msg = {
        "id": 2,
...
def calculate_merkle_root(coinbase_hash, merkle_branch):
    ...
    for h in merkle_branch:
        ...
        current_hash = hashlib.kawpow(hashlib.kawpow(combined).digest()) # Reassign current_hash to hash object
    return binascii.hexlify(current_hash.digest()[::-1]).decode() # Use digest() and reverse for final output

def mine_kawpow(sock, username, extranonce1, extranonce2_size):
        ...
        while True:
            ...
            for data in messages:
                if data.get("method") == "mining.notify":
                    ...
                    coinbase_hash = hashlib.kawpow(hashlib.kawpow(coinbase_tx).digest())
                    # Compute Merkle root
                    ...
                    while time.time() - start_time < 10:
                        ...
                        # Double SHA-256
                        hash_result = hashlib.kawpow(hashlib.kawpow(block_header).digest()).hexdigest()
...
def main_kawpow():
    ...
    sub_res, extranonce1, extranonce2_size = subscribe_kawpow(sock)
    authorize_kawpow(sock, USERNAME, PASSWORD)
    mine_kawpow(sock, USERNAME, extranonce1, extranonce2_size)
