...
HOST = 'power2b.sea.mine.zpool.ca'
PORT = 6242
...
def authorize_power2b(sock, username, password):
    print("🔑 Authorizing...")
    auth_msg = {
        "id": 2,
...
def calculate_merkle_root(coinbase_hash, merkle_branch):
    ...
    for h in merkle_branch:
        ...
        current_hash = hashlib.power2b(hashlib.power2b(combined).digest()) # Reassign current_hash to hash object
    return binascii.hexlify(current_hash.digest()[::-1]).decode() # Use digest() and reverse for final output

def mine_power2b(sock, username, extranonce1, extranonce2_size):
        ...
        while True:
            ...
            for data in messages:
                if data.get("method") == "mining.notify":
                    ...
                    coinbase_hash = hashlib.power2b(hashlib.power2b(coinbase_tx).digest())
                    # Compute Merkle root
                    ...
                    while time.time() - start_time < 10:
                        ...
                        # Double SHA-256
                        hash_result = hashlib.power2b(hashlib.power2b(block_header).digest()).hexdigest()
...
def main_power2b():
    ...
    sub_res, extranonce1, extranonce2_size = subscribe_power2b(sock)
    authorize_power2b(sock, USERNAME, PASSWORD)
    mine_power2b(sock, USERNAME, extranonce1, extranonce2_size)
